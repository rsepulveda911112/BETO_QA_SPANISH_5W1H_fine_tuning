...
from src.common.metrics import remove_punc
...
class QuestionAnsweringModel:
    def __init__(
        ...
        if model_type == "auto":
            self.tokenizer = tokenizer_class.from_pretrained(model_name, use_fast=False, **kwargs)
        else:
    ...
    def predict(self, to_predict, n_best_size=None):
        ...
        return answer_list, probability_list, all_predictions
    ...
    def calculate_results(self, truth, predictions, **kwargs):
        ...
        for q_id, answer in truth_dict.items():
            ...
            if True:
                prediction_value = remove_punc(predictions[q_id].strip())
                answer_value = remove_punc(answer.strip())
            else:
                prediction_value = predictions[q_id].strip()
                answer_value = answer.strip()

            if prediction_value == answer_value:
                correct += 1
                correct_text[q_id] = answer
            elif (
                prediction_value in answer_value
                or answer_value in prediction_value
            ):
                similar += 1
                similar_text[q_id] = {
                    "truth": answer,
