...
class Keyphrase:
    def __init__(self, sentence, label, id, spans, text=''):
...
class Attribute:
    def __init__(self, keyphrase: Keyphrase, label, value):
    ...
    def __repr__(self):
        return "Attribute(label=%r)" % (self.label, self.value)
...
class Sentence:
    ...
    def load(finput) -> "List[Sentence]":
        ...
        list_ex = []
        for s in finput.read_text(encoding="utf8").splitlines():
            if s.__contains__('El agua caliente que se bebe ya sea con limón o con ramas'):
                print('')
            if s:
                sen = Sentence(s.strip())
            else:
                sen = Sentence(' ')
            list_ex.append(sen)
        return list_ex
...
class CollectionV2Handler(CollectionHandler):
    ...
    def load(
        ...
        *,
        ...
        # sentences_length[0] = sentences_length[0]+1
        for i in range(1, len(sentences_length)):
            if sentences_length[i] == 1:
                sentences_length[i] += sentences_length[i - 1]
            #     sentences_length[i] += sentences_length[i - 1]
            #     if sentences_length[i - 1] == 2 or i >= 2 :
            #         if sentences_length[i - 1] - sentences_length[i - 2] == 2:
            #             sentences_length[i - 1] -= 1
            # sentences_length[i] += sentences_length[i - 1] + 1
            else:
                sentences_length[i] += sentences_length[i - 1] + 1
        ...
        for ann in ann_file.annotations:
            if isinstance(ann, EntityAnnotation):
                ...
                if not 0 <= sid <= len(sentences):
                    warnings.warn("Wrong sentence id %r" % sid)
                    continue

                sentence = sentences[sid]

                keyphrase = Keyphrase(sentence, ann.type, tid, spans, ann.text)
        ...
        for ann in ann_file.annotations:
            if isinstance(ann, RelationAnnotation) and relations:
                ...
                attribute = Attribute(keyphrase, ann.type, ann.value)
    ...
    def _load_input(cls, collection: Collection, finput: Path) -> List[Sentence]:
        if str(finput).__contains__('14-Informacion.txt'):
            print('')
    ...
    def _get_relative_ann(cls, spans, sentences_length: List[int]) -> int:
        ...
        if i > 0:
            ...
            # spans_new = []
            # for start, end in spans:
            #     new_start = start - sentences_length[i - 1] - 1,
            #     end_start = end - sentences_length[i - 1] - 1,
            #
            #     spans_new.append((new_start, end_start))
            spans.sort(key=lambda t: t[0])
            # spans = spans_new
        return i, spans
