...
class EventAnnotation:
    def __init__(self, id, typ, ref, args, attris):
        self.id = id
        self.type = typ
        self.ref = ref
        self.args = args
...
class AttributeAnnotation:
    def __init__(self, id, typ, ref, value):
        self.id = id
        self.type = typ
        self.ref = ref
        self.value = value
    ...
    @staticmethod
    def parse(line):
        if line.__contains__('Main-Event'):
            id, typ, ref = line.strip().split()
            value = None
        else:
            id, typ, ref, value = line.strip().split()
        return AttributeAnnotation(id, typ, ref, value)
    ...
    def __repr__(self):
        return "<Attribute(id=%r, type=%r, ref=%r)>" % (self.id, self.type, self.ref, self.value)

    def as_brat(self):
        return "%s\t%s %s" % (self.id, self.type, self.ref, self.value)
